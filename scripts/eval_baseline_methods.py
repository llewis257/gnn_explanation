...
def eval_var(traffic_reading_df, n_lags=3):
    ...
    for i, horizon in enumerate(n_forwards):
        rmse = masked_rmse_np(preds=y_predicts[i].to_numpy(), labels=y_test.to_numpy(), null_val=0)
        mape = masked_mape_np(preds=y_predicts[i].to_numpy(), labels=y_test.to_numpy(), null_val=0)
        mae = masked_mae_np(preds=y_predicts[i].to_numpy(), labels=y_test.to_numpy(), null_val=0)
